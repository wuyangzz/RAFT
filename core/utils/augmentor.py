...
class SimulationsAugmentor:
    ...
    def spatial_transform(self, img1, img2, flow,mask):
        # randomly sample scale
        ht, wd = img1.shape[:2]
        min_scale = np.maximum(
            (self.crop_size[0] + 8) / float(ht), 
        ...
        if ht<self.crop_size[0] or wd<self.crop_size[1]: 
            ...
            flow = cv2.resize(flow, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            flow = flow * [scale_x, scale_y]
        elif np.random.rand() < self.spatial_aug_prob:
            # rescale the images
            img1 = cv2.resize(img1, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
            mask = cv2.resize(mask, None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR)
    ...
    def __call__(self, img1, img2, flow,mask):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
